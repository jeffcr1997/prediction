                ...
                if st.session_state.model_info['input_fields']:
                    for field in st.session_state.model_info['input_fields']:
                        ...
                        if field_type in ['integer', 'int']:
                            value = st.number_input(f"{field_name}", value=0, step=1, format="%d", key=field_name)
                            if value < 0:
                                valid = False
                                error_messages.append(f"'{field_name}' no puede ser negativo.")
                            input_data[field_name] = value
                        elif field_type in ['double', 'float']:
                            value = st.number_input(f"{field_name}", value=0.0, format="%.6f", key=field_name)
                            if value < 0:
                                valid = False
                                error_messages.append(f"'{field_name}' no puede ser negativo.")
                            input_data[field_name] = value
                        elif field_type == 'string':
                            value = st.text_input(f"{field_name}", key=field_name)
                            if value.strip() == "":
                                valid = False
                                error_messages.append(f"'{field_name}' no puede estar vacío.")
                            input_data[field_name] = value
                        else:
                            value = st.number_input(f"{field_name}", value=0.0, format="%.6f", key=field_name)
                            if value < 0:
                    ...
                    for i in range(int(num_fields)):
                        ...
                        with col1:
                            field_name = st.text_input(f"Nombre campo {i+1}", value=f"feature_{i+1}", key=f"name_{i}")
                        with col2:
                            field_value = st.number_input(f"Valor {i+1}", value=0.0, format="%.6f", key=f"value_{i}")
                        
                        if field_name.strip() == "":
                            valid = False
                            error_messages.append(f"Nombre del campo {i+1} no puede estar vacío.")
                        elif field_value < 0:
                            valid = False
                            error_messages.append(f"'{field_name}' no puede ser negativo.")
